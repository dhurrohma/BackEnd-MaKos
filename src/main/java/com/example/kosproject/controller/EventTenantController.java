...
//        EventTenant eventTenant = modelMapper.map(eventTenantRequest, EventTenant.class);
    ...
    @PostMapping("/remove")
    public ResponseEntity remove(@RequestBody @Valid EventTenant eventTenant) throws Exception {
//        EventTenant eventTenant = modelMapper.map(eventTenantRequest, EventTenant.class);
        eventTenantService.remove(eventTenant);
        return ResponseEntity.status(HttpStatus.CREATED).body(new SuccessResponse<>("Success add event-tenant", "removed"));
    }
