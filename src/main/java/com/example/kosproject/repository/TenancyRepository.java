...
//    @Query("SELECT t FROM Tenancy t WHERE t.startDate = :startDate")
    ...
    @Query("SELECT count(t) FROM Tenancy t where t.roomPrice.room.roomId = :roomId and bookDate <= :now and startDate <= :now and endDate = :date")
    Long occupied(Integer roomId, Date now);
    List<Tenancy> findByUser_UserId(Integer userId);
}
