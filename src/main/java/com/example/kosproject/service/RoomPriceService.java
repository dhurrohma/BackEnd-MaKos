...
public class RoomPriceService implements IRoomPriceService{
    ...
    @Override
    public Page<RoomPrice> findAllNewest(Integer page, Integer size, String direction, String sortBy) {
        Sort sort = Sort.by(Sort.Direction.valueOf(direction), sortBy);
        Pageable pageable = PageRequest.of((page - 1), size, sort);
        Page<RoomPrice> roomPrices = roomPriceRepository.findByNewest(pageable, true);
        return roomPrices;
    }
