...
public class TenancyService implements ITenancyService{
    ...
    public Tenancy create(Tenancy tenancy) throws Exception {
        try {
            ...
            isoFormat.setTimeZone(TimeZone.getTimeZone("Asia/Jakarta"));
            Date now = isoFormat.parse(isoFormat.format(new Date()));
            tenancy.setBookDate(now);
            Tenancy saveTenancy = tenancyRepository.save(tenancy);
            Integer roomId = saveTenancy.getRoomPrice().getRoom().getRoomId();
...
//        SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd");
    ...
    public void updateById(Tenancy tenancy, Integer id) throws Exception {
        try {
            ...
            Integer roomId = existTenancy.getRoomPrice().getRoom().getRoomId();
            setAvailableRoom(roomId);
    ...
    public List<Tenancy> findByUserId(Integer userId) throws Exception {
        List<Tenancy> tenancyList = tenancyRepository.findByUser_UserId(userId);
    ...
    public Long occupied(Integer roomId){
        try {
            SimpleDateFormat isoFormat = new SimpleDateFormat("yyyy-MM-dd");
            isoFormat.setTimeZone(TimeZone.getTimeZone("Asia/Jakarta"));
            Date now = isoFormat.parse(isoFormat.format(new Date()));
            return tenancyRepository.occupied(roomId, now);
        } catch (Exception e) {
            return 0L;
        }
    }

    public void setAvailableRoom(Integer roomId){
        try {
            Room room = roomService.findById(roomId);
            room.setRoomAvailable(occupied(roomId) <= 0);
            roomService.updateById(room, roomId);
        } catch (Exception e) {
            throw new RuntimeException("Delete failed");
        }
    }
