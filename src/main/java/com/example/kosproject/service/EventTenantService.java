...
public class EventTenantService implements IEventTenantService{
    ...
    public void remove(EventTenant eventTenant) throws Exception {
        try {
            Integer uid = eventTenant.getUser().getUserId();
            Integer eid = eventTenant.getEvent().getEventId();
            List<EventTenant> eventTenants = eventTenantRepository.findByPo(uid, eid);
            eventTenants.forEach(n -> eventTenantRepository.delete(n));
        } catch (DataIntegrityViolationException e) {
            throw new EntityExistException();
        }
    }
